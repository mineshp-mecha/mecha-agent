...
use agent_settings::{read_settings_yml, AgentSettings, GrpcSettings};
...
async fn main() -> Result<()> {
    ...
    let logger_provider = match init_logs_config(otlp_collector_endpoint) {
    ...
    let tracing_bridge_layer = layer::OpenTelemetryTracingBridge::new(&logger_provider);
    ...
    let _ = init_handlers(settings, &socket_addr).await;
    Ok(())
}
